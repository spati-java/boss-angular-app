"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var policy_model_1 = require("./policy.model");
var policy_service_1 = require("./policy.service");
var PolicyComponent = (function () {
    function PolicyComponent(service) {
        this.service = service;
        this.model = new policy_model_1.Policy('', '', 0, '', '');
    }
    PolicyComponent.prototype.ngOnInit = function () {
        this.getAllPolicies();
    };
    PolicyComponent.prototype.getAllPolicies = function () {
        var _this = this;
        this.service.getAllPolciies().subscribe(function (policies) { return _this.policies = policies; });
    };
    PolicyComponent.prototype.save = function () {
        var _this = this;
        this.service.createPolicy(this.model).subscribe(function (data) { return _this.savedData = data; });
    };
    PolicyComponent = __decorate([
        core_1.Component({
            moduleId: module.id,
            templateUrl: 'policy.component.html',
            styleUrls: ['policy.component.css'],
            providers: [policy_service_1.PolicyService]
        }), 
        __metadata('design:paramtypes', [policy_service_1.PolicyService])
    ], PolicyComponent);
    return PolicyComponent;
}());
exports.PolicyComponent = PolicyComponent;
//# sourceMappingURL=policy.component.js.map